...
def run_janus(algorithm,isdirected,isweighted,ismultigraph,dependency,output,kmax,klogscale,krank):
    ...
    janus.createHypothesis('M1: same-country',getMatrix(['same-country'],output))
    janus.createHypothesis('M2: nominations',getMatrix(['nominations'],output))
    janus.createHypothesis('M3: first-movie',getMatrix(['first-movie'],output))
    # janus.createHypothesis('M3: first-movie-decade',getMatrix(['first-movie-decade'],output))
    janus.createHypothesis('M4: transitivity',getMatrix(['transitivity'],output))
